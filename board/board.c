...
void board_print_clock_freq(void)
{
    ...
    printf("==============================\n");
    printf("gptmr4:\t %dHz\n", clock_get_frequency(clock_gptmr4));
    printf("gptmr5:\t %dHz\n", clock_get_frequency(clock_gptmr5));
    printf("==============================\n");
}
...
void board_init_uart(UART_Type *ptr)
{
    ...
    init_uart_pins(ptr);
    uint32_t freq = board_init_uart_clock(ptr);
    printf("UART clock freq: %d\n", freq);
}
...
|  |.-----..-----.|  |_.--.--.    .'  _|.-----..----.\n\
|  ||  -__||  -__||   _|_   _|    |   _||  _  ||  __|\n\
|__||_____||_____||____|__.__|    |__|  |_____||____|\n\
----------------------------------------------------------------------\n" };
...
static void board_turnoff_rgb_led_core0(void)
{
...
{
    board_turnoff_rgb_led_core0();
    init_led_pins_as_gpio_core0();
    gpio_set_pin_output_with_initial(HPM_GPIO0, GPIO_DI_GPIOB, 24, board_get_led_gpio_off_level());
}
...
void board_led_toggle_core0(void)
{
    ...
    if (0 == i % 2)
    {
        gpio_write_pin(HPM_GPIO0, GPIO_DI_GPIOB, 24, BOARD_LED_OFF_LEVEL);
    }else
    {
        gpio_write_pin(HPM_GPIO0, GPIO_DI_GPIOB, 24, BOARD_LED_ON_LEVEL);
    }
    i++;
#else
    gpio_toggle_pin(BOARD_LED_GPIO_CTRL, BOARD_LED_GPIO_INDEX, BOARD_LED_GPIO_PIN);
...
uint32_t board_init_uart_clock(UART_Type *ptr)
{
    uint32_t freq = 0U;
    if (ptr == HPM_UART0) {
        clock_set_source_divider(clock_uart0, clk_src_pll1_clk0, 1);
        clock_add_to_group(clock_uart0, 0);
        freq = clock_get_frequency(clock_uart0);
    } else if (ptr == HPM_UART1) {
        clock_set_source_divider(clock_uart1, clk_src_pll1_clk0, 5);
        ...
        clock_set_source_divider(clock_uart4, clk_src_pll1_clk0, 5);
        clock_add_to_group(clock_uart6, 0);
